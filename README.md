...
| `tools/align-rig.py` | Locks the rig stages onto one head (see the assembly rig, below) |
...
- **They share no source framing** and never will: feet sat at 92.0% / 98.2% / 98.0% of three different canvases, one of them 912×1171 rather than square. They're normalised onto one 900×1125 canvas.
- **The head is the anchor, and `tools/align-rig.py` sets it.** Hand-tuned scales left the lion zooming and sliding between steps: the mane measured **384, 416, 442 and 472px wide** across the four stages, with head centres up to 100px apart. The tool measures each stage from its alpha silhouette — the head is the widest run of rows in the top 45%, cut off at the neck, where the silhouette narrows to 85% and starts widening into the shoulders — then applies one uniform scale and one translation so every head lands in the same place at the same size. After it: head widths **372–387**, centres within **1px**, shoulder lines within **7px**. Run `python3 tools/align-rig.py --check` to see the numbers without writing.
- **Match the head on its DIAGONAL, not its width.** The skull stage's mane is rounder than the rest; matching width alone left it 8% too tall.
- **The skeleton has no head**, so it matches on body length — shoulder line to floor — and lands its shoulders where the maned stages' necks are. Its body then measures 430 against the armoured stage's 434.
- **The shoulder line is the neck row**, not "the widest row below the head": arms, tails and legs keep widening downwards, so the widest row lands on the hips and the skeleton comes out 18% too small.
...
- **Nothing is upscaled.** The smallest head (the armoured stage) is the reference, so every other stage scales down; the group is then centred on the canvas at its natural size.
- **What alignment cannot fix: the bodies.** Once the heads agree, each body is whatever the generator drew against that head, and they disagree — measuring shoulders-to-floor, the armoured stage and the skeleton are 434 and 430, the skilled and awake stages 322 and 329, and the skull stage **292**. So the feet sit up to 137px apart while the head never moves. Only new art fixes that, generated against the reference frame (see below).
- **Earlier automatic landmarks that failed** and should not be tried again: total height (oversizes the headless skeleton), figure width (the skeleton's splayed arms vs the lion's tail and aura), leg span (contaminated by hanging hands in some frames and not others), leg length (armour lowers the visible crotch, giving 313/287/**195**), and hip width (skeletal bodies are open, so the run at the midline is a thin strut — 58/152/233/**21**/88, i.e. scale factors up to 14×).

**Regenerating a stage so it drops straight in:** generate **from the aligned art**, not from a text prompt alone — ask for the one change and nothing else, so the pose, framing and head size come back untouched. Feed the stage you are replacing (or `apex-rig-3-face.webp`, the reference) as the input image, keep the 900×1125 frame and the transparent background, and hold the head where it is: mane box **384×367 centred at (413, 349)**, shoulder line **y=532**, floor **y=966**. Then run `python3 tools/align-rig.py` anyway — it costs nothing and it catches whatever drifted.
