...
| `css/gtamex.css` | GTAmex (section 06), the shared parts: title, HUD, the drone flight on a framed screen with a glow sampled from the video, the REC dot, mission passed — and the **classic** look, the mission-select grid (scoped to `data-look="classic"`) |
| `css/gtamex-cover.css` | GTAmex's **cover** look: the clips and stills as GTA box art round the logo, assembled on scroll |
| `js/gtamex.js` | Everything that moves in GTAmex, for both looks: the cover's assembly, the classic entrances, the HUD, the footage playing only on screen, the flight's bar and glow |
...
A side quest: a GTA-style game of the office, built with APEX in co-op. No copy — the footage carries it. **Two looks, one markup**: `data-look` on the section says which, and `?gta=classic` / `?gta=cover` in the URL overrides it (an inline script in the section, before anything is laid out), so the two can be compared live in the room.

- **The cover** (the default) turns the clips and stills into **GTA box art**: fourteen panels of real footage round the logo, black gutters, one composed picture — the language of every GTA cover. It **assembles as you scroll**: each panel flies in from outside the box, spun and oversized, and locks with a flash; the box comes up under them; the logo slams into the middle, the box shakes once and a gloss crosses it; the wanted stars and the cash fill as it comes together, sirens at the end. Then it holds, alive — every clip playing, the stills drifting — before the flight. Sticky (the run is 220svh, the stage a screen tall), below every pin, so nothing above moves; the assembly starts as the stage is about to stick, so the heading still shows an empty stage, and finishes 80% of a screen later.
- **The classic** is the mission-select grid it replaced, kept whole as the backup: the logo drops in, the tiles deal in, the HUD fills over the grid. Its CSS is scoped to `data-look="classic"`; the cover's wrappers (`.gta__run`, `.gta__stage`) are `display: contents` there and its logo panel is hidden.
- **The logo panel is the section's own logo, moved** (`js/gtamex.js` moves the logo and its synthwave floor into the middle panel and the HUD onto the stage, once, before any trigger measures): on a night sky over a sliced synthwave sun. It sits **last** in the grid so classic's every-third-tile colours count the same tiles; the cover places panels by `grid-template-areas`, so order doesn't matter there.
- **Panels move on `transform` (GSAP), hover on `translate` (CSS)**; the flash is `--flash` (GSAP) drawn by `::after` (CSS), and the box's black and shadow are `::before` at `--box`, so the assembly can bring the box up under the panels instead of leaving an empty slab. The flash is a keyframed 0 → 0.7 → 0, never a from/to: a scrubbed from-state would sit every panel at white when scrolling back.
- **Positions from the layout, never the box** (`offsetLeft`, the grid is the panels' offsetParent), as function values with `invalidateOnRefresh`, because GSAP is moving the boxes being measured.
- **Presenter mode**: one stop on the finished box — a click glides through the assembly, so it plays on the way — then the flight, then mission passed (`apexGta.beats()`). Classic keeps its heading and grid stops.
- **Phones** get the cover as a two-across picture read top to bottom, the logo panel first; **reduced motion** gets the cover still and whole in the page (no sticky run). Both fill the HUD as it scrolls past.

On the user's notes, for both looks:

- **No overlay on any footage.** The CRT scanlines over the whole section read as degraded video and are gone; classic keeps only its corner brackets and the dark fade under the names. The cover's only overlay is a short fade under each name.
- **On top, only the REC dot**: the mission numbers, timers and titles beside it went. It blinks on clips and holds on stills.
- **Names are smaller**, and two were wrong: the dining clip is **Food Court** and the corridor **Lift Lobby** (the game's own zone labels say CAFETERIA and ELEVATOR LOBBY; the user's names are what's on screen). The "Floor walk" still is the same lift lobby, so its alt text no longer says meeting rooms.
- **Everything is re-cut from the reel at 1334×750** instead of 720×404 (upscaled into big tiles, it was soft): each output frame is the reel frame the old clip was matched to, so timing, speed-ups and cuts are unchanged. The clips went from 1.7 MB to about 7 MB in all; they're `preload="none"` and play only on screen.
- **The flight's cinema bars are full width now.** The game draws its letterbox as rounded bars inset from the frame, with the scene showing above the top one, below the bottom one and in their corners — it read as a glitch. `drone.mp4` is re-encoded from the untouched recording with square black bars baked in from the frame's edges: the top bar to the bottom of the game's own (82 of 720 rows), the bottom one deeper (102) to take the game's "press any key to skip" hint with it, which sat just above its band. **The bars track the game's frame by frame** — measured per frame from the recording as they grow in (3.0–3.55 s) and retract (34.0–34.48 s) — so the cut to gameplay at the end is exactly as it was. Burnt in rather than drawn over the video, so they hold on every browser, the poster and a paused frame.
