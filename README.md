...
- **What travels is a card, and it changes at every stop** (`journey` in `content.js`, one per packet, same order): *Sunday, 9:30 AM · a gut-healthy breakfast* → *a job for MISO* → the shopping list, four boxes → *fill the Zepto cart* → *3 items · ₹117 · pear not found · nothing ordered* → the Telegram message. The dot with a mono tag that was here said *that* something moved; this says what each layer **adds**, which is the point of the section. Words from MISO's real Sunday run.
- **Each stop takes the card in and sends out a ring** (`.ping`, one per arrival, placed like a node): the card comes out of the stop it leaves (`scale` from 0.3), rides the wire centred on it, and shrinks into the next stop as that stop's ring goes out. GSAP owns the card's transform and opacity; the card has no CSS transition on either.
- **The last card comes to rest beside you** (`DOCK`, above the phone) and the whole route turns gold (`data-done`) — the circle is closed. That is the section's final presenter beat.
- **The timeline is still exactly 5.7 s** (`legs` untouched; the rings and the dock fit inside the old padding), so presenter mode's arrival fractions still land on each arrival. Change the length and change those.
- **A card is wider than the route is from the stage's edge**, so `place()` clamps its centre to keep it on the stage; on the long way home it hugs the edge instead of hanging over the captions.
- **No journey to play (phones, reduced motion)**: the stage shows where it ends — the Telegram card inside the gold loop. Beside you, as on desktop, it covered the skills arc on a phone-sized stage.
- **The stage can't grow much at 1440×900**: the screen's height caps it (`100vh - 310px`, at 1200:700), and a captions row above it would cost more height than it frees width. The captions column went from 240–300px to 220–250px, which gave the stage ~8%; the cards size in `cqw` with a 11px floor.
