...
- **It runs off a continuous position between steps**, not the latched index — read from the same `top 55%` line the latches use, and interpolated per step, so the steps' unequal heights don't make the middle ones converge faster than the ends.
- **That position is eased, or the picture changes before its words do.** The step *latches* when the next step's top reaches the line, but the raw position between two step tops is linear — so the figure was half changed at the halfway mark, a whole half-step early, and it read as the rig running ahead of the copy. Cubing the fraction (`HOLD`) holds each stage at full strength until the next step is nearly up, then swaps: measured, the outgoing stage is still at 98% opacity at 60% of the way across, crosses between 70% and 95%, and is done exactly as the step latches.
...
- **`is-on` and the convergence are different jobs.** `is-on` is the latch, and it is the only thing the effects listen to, so none of them burn a frame while their step is off screen. The convergence moves and fades layers *between* those latches, and it owns `opacity`, `transform` and `filter` outright — which is why `.rig.is-converging` kills the CSS transition on those properties. Left on, the transition smears each frame into the next and the figure arrives a beat after its step. Same one-owner-per-property rule as the hero and the phone.
...
- **The brain glow is screened** (`mix-blend-mode: screen`), so it lights the gold art underneath instead of laying a grey disc on top of it. It carries the 01→02 transition on its own (see `FLAT` above), so it is a pulse *and* a shine: a specular band travelling across the core, clipped to the core's own ellipse so it never runs out onto the skull. The band rests off screen for most of its loop — sweeping back and forth without pause reads as a scanner; passing now and then reads as a shine.
...
- **The night band on the rim is gone.** It ran 21:00–06:00, which is one arc only on a 24-hour face; on a 12-hour one it would cross both halves. The sun/moon in the middle says it instead, which is also the thing the face itself cannot: **9 o'clock is two different times of day**. Both icons sit stacked in one box, so day↔night is a cross-fade in place and the centre's grid never reflows.
